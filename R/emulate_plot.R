...
.plot_ps <- function(obj, title) {
  # Estimand guard
  estimand <- obj$settings$estimand
  if (estimand %in% c("PP", "AT")) {
    stop("type 'ps' not applicable with ", estimand, " estimand.\n",
         "The saved score is a per-period adherence probability, not a baseline PS.",
         call. = FALSE)
  }
  if (estimand == "ITT") {
    stop("type 'ps' not applicable with ITT estimand (no PS estimated).",
         call. = FALSE)
  }
...
.plot_equipoise <- function(obj, title) {
  ...
  estimand <- obj$settings$estimand
  if (estimand %in% c("PP", "AT")) {
    stop("type 'equipoise' not applicable with ", estimand, " estimand.\n",
         "PP/AT cloning forces treatment prevalence to exactly 0.5.",
         call. = FALSE)
  }
  if (estimand == "ITT") {
    stop("type 'equipoise' not applicable with ITT estimand (no PS estimated).",
         call. = FALSE)
  }
