...
#'   \item{\code{_emulate_tv_<varname>}}{For each covariate listed in
#'     \code{covariates} (the time-varying covariates set in
#'     \code{\link{emulate_prepare}}), a copy of the original time-varying
#'     values is saved \strong{before} covariate freezing. These columns are
#'     used automatically by \code{\link{emulate_weight}} so that the IP
#'     weight models condition on time-varying \eqn{L_t}, while the outcome
#'     model in \code{\link{emulate_fit}} uses the frozen baseline values
#'     \eqn{L_0}. If your data already contains a column with one of these
#'     names, it will be overwritten.}
#' }
#'
...
emulate_expand <- function(obj, trials = NULL, maxfollowup = 0L, grace = 0L) {
  ...
  for (t in trial_periods) {
    ...
    # Weight models need time-varying values (L_t) to model treatment
    # switching; the outcome model needs frozen baseline values (L_0).
    data.table::setorderv(trial_dt, c(id_col, period_col))
    if (length(covariates) > 0) {
      for (v in covariates) {
        tv_col <- paste0(prefix, "tv_", v)
        trial_dt[, (tv_col) := get(v)]
      }
    }

    # Freeze all covariates at baseline (trial-entry) values for outcome model
