...
.check_matched <- function(obj) {
  .check_expanded(obj)
  if (!isTRUE(obj$state$matched)) {
    stop("Data has not been matched. Run emulate_match() first.", call. = FALSE)
  }
}

.check_stratified <- function(obj) {
  .check_expanded(obj)
  if (!isTRUE(obj$state$stratified)) {
    stop("Data has not been stratified. Run emulate_stratify() first.", call. = FALSE)
  }
}

.check_calibrated <- function(obj) {
  .check_fitted(obj)
  if (!isTRUE(obj$state$calibrated)) {
    stop("Results have not been calibrated. Run emulate_calibrate() first.", call. = FALSE)
  }
}
