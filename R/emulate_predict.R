...
#' @param att A logical value. If \code{TRUE}, the reference population is
#'   restricted to individuals who initiated treatment at baseline (ATT =
#'   average treatment effect on the treated). Default is \code{FALSE} (ATE =
#'   average treatment effect over all eligible individuals).
#'
...
emulate_predict <- function(obj, times, type = "cum_inc", samples = 100L,
  ...
  if (!obj$model$type %in% c("logistic")) {
    stop("emulate_predict currently only supports logistic model. ",
         if (obj$model$type == "poisson") "Poisson models use a log link incompatible with G-formula survival predictions."
         else paste0("Model type '", obj$model$type, "' is not supported."),
         call. = FALSE)
  }
  ...
  target_label <- if (att) "ATT (treated at baseline)" else "ATE (all eligible)"

  message("emulate_predict - Marginal Predictions")
  message(strrep("-", 50))
  message("Type:          ", type)
  message("Target:        ", target_label)
  ...
  # De-clone: keep arm=0 explicitly (both clones have identical baseline covariates)
  arm_col <- paste0(prefix, "arm")
  ref <- data.table::copy(dt[get(fu_col) == 0 & get(arm_col) == 0])
  ...
  if (att) {
    ...
    ref <- ref[get(treat_col) == 1]
  }

  if (nrow(ref) == 0L) {
    msg <- "Reference population is empty (no observations at followup=0 in estimation sample)."
    if (att) msg <- paste(msg, "Check that treated observations exist at baseline.")
    stop(msg, call. = FALSE)
  }
  ...
  obj$prediction_meta <- list(type = type, samples = samples,
                               level = level, target = if (att) "ATT" else "ATE",
                               att = att, mc_0 = mc_0, mc_1 = mc_1)
