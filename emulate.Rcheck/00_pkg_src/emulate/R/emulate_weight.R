...
emulate_weight <- function(obj, switch_d_cov = NULL, switch_n_cov = NULL,
  ...
  if (!is.null(ps_trim)) {
    ...
    keep <- .trim_ps(dt$.ps_trim_val, ps_trim)
    n_ps_trimmed <- sum(!keep, na.rm = TRUE)

    if (n_ps_trimmed > 0) {
      message("  PS trimming at [", ps_trim[1], ", ", ps_trim[2],
              "]: removed ", n_ps_trimmed, " observations")
      dt <- dt[keep]
    } else {
