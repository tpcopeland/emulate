...
#' @param equipoise A logical value. If \code{TRUE} and propensity scores are
#'   available (from weighting, matching, or stratification), computes
#'   preference scores and the percentage of observations in the equipoise
#'   range. Default is \code{FALSE}.
#' @param equipoise_bounds A numeric vector of length 2 giving the preference
#'   score bounds for clinical equipoise (default \code{c(0.3, 0.7)}).
#'
...
emulate_diagnose <- function(obj, balance_covariates = NULL, by_trial = FALSE,
                              equipoise = FALSE, equipoise_bounds = c(0.3, 0.7)) {
  .check_expanded(obj)
  ...
  if (!is.null(balance_covariates) && length(balance_covariates) > 0) {
    ...
    if (estimand %in% c("PP", "AT")) {
      # PP/AT: cloning makes arm comparison mechanically zero.
      # Compute unweighted SMDs on de-cloned baseline data using observed treatment.
      message("\nCovariate Balance (SMD):")
      message("  Unweighted: observed treatment at baseline (de-cloned)")
      if (has_weights) message("  Weighted: arm comparison with IP weights (full data)")
      ...
      # De-clone: keep arm=0 at baseline (both clones have identical covariates)
      baseline_idx <- dt[[arm_col]] == 0 & dt[[fu_col]] == 0
      dt_bl <- dt[baseline_idx]
      treat1_bl <- dt_bl[[treat_col]] == 1
      treat0_bl <- dt_bl[[treat_col]] == 0

      for (i in seq_along(balance_covariates)) {
        v <- balance_covariates[i]
        x_bl <- dt_bl[[v]]

        m1 <- mean(x_bl[treat1_bl], na.rm = TRUE)
        m0 <- mean(x_bl[treat0_bl], na.rm = TRUE)
        v1 <- var(x_bl[treat1_bl], na.rm = TRUE)
        v0 <- var(x_bl[treat0_bl], na.rm = TRUE)
        pooled_sd <- sqrt((v0 + v1) / 2)
        balance$smd_unwt[i] <- if (pooled_sd > 0) (m1 - m0) / pooled_sd else 0
      }

      # Weighted SMDs: arm comparison is meaningful after IPTW
      if (has_weights) {
        arm0_bl <- dt[[arm_col]] == 0 & dt[[fu_col]] == 0
        arm1_bl <- dt[[arm_col]] == 1 & dt[[fu_col]] == 0
        for (i in seq_along(balance_covariates)) {
          v <- balance_covariates[i]
          x <- dt[[v]]
          wm0 <- weighted.mean(x[arm0_bl], w[arm0_bl], na.rm = TRUE)
          wm1 <- weighted.mean(x[arm1_bl], w[arm1_bl], na.rm = TRUE)
          wv0 <- .weighted_var(x[arm0_bl], w[arm0_bl])
          wv1 <- .weighted_var(x[arm1_bl], w[arm1_bl])
          wpooled_sd <- sqrt((wv0 + wv1) / 2)
          balance$smd_wt[i] <- if (wpooled_sd > 0) (wm1 - wm0) / wpooled_sd else 0
        }
      }

      for (i in seq_along(balance_covariates)) {
        smd_wt_val <- if (has_weights) abs(balance$smd_wt[i]) else NA
        message(sprintf("  %-20s %10.4f %10.4f %10.1f",
                        balance_covariates[i], abs(balance$smd_unwt[i]),
                        if (is.na(smd_wt_val)) 0 else smd_wt_val, 0.1))
      }
    } else {
      # ITT: arm reflects real assignment — standard comparison
      message("\nCovariate Balance (SMD):")
      message(sprintf("  %-20s %10s %10s %10s", "Covariate",
                      "Unweighted", "Weighted", "Threshold"))
      message(strrep("-", 55))
      ...
      for (i in seq_along(balance_covariates)) {
        ...
        message(sprintf("  %-20s %10.4f %10.4f %10.1f",
                        v, abs(balance$smd_unwt[i]),
                        if (has_weights) abs(balance$smd_wt[i]) else 0, 0.1))
      }
    }

    max_smd_col <- if (has_weights) "smd_wt" else "smd_unwt"
    max_smd <- max(abs(balance[[max_smd_col]]), na.rm = TRUE)
    balanced <- max_smd < 0.1
    message(strrep("-", 55))
    if (has_weights) message("Max weighted SMD: ", sprintf("%.4f", max_smd))
    message("Balance achieved: ", if (balanced) "Yes" else "No")
  }

  # Equipoise diagnostics (optional)
  equipoise_result <- NULL
  if (equipoise) {
    estimand <- s$estimand

    if (estimand %in% c("PP", "AT")) {
      stop("Equipoise is not applicable with the ", estimand, " estimand.\n",
           "PP/AT cloning forces treatment prevalence to exactly 0.5.\n",
           "The saved propensity score is a per-period adherence probability,\n",
           "not a baseline propensity score suitable for equipoise.\n",
           "Use balance_covariates instead.", call. = FALSE)
    }

    if (estimand == "ITT") {
      stop("Equipoise is not applicable with the ITT estimand.\n",
           "ITT analysis sets all weights to 1 - no propensity score exists.\n",
           "Use balance_covariates instead.", call. = FALSE)
    }

    # Find PS values from weights, matching, or stratification
    ps_vals <- NULL
    if (!is.null(obj$weights$ps_values)) {
      ps_vals <- obj$weights$ps_values
    } else if (!is.null(obj$matching$ps_values)) {
    ...
    if (is.null(ps_vals)) {
      message("\nEquipoise: No propensity scores available. ",
              "Use ps_method in emulate_weight(), emulate_match(), or emulate_stratify().")
    } else {
      ps_vals <- ps_vals[!is.na(ps_vals)]

      # Get treatment arm info for proportion
      arm_vals <- dt[[arm_col]]
      if (length(ps_vals) == length(arm_vals)) {
        prop_treated <- mean(arm_vals, na.rm = TRUE)
      } else {
        prop_treated <- 0.5
      }

      # Compute preference scores
      pref_scores <- .compute_preference_score(ps_vals, prop_treated)

      # Proportion in equipoise
      in_equip <- pref_scores >= equipoise_bounds[1] &
                  pref_scores <= equipoise_bounds[2]
      pct_equipoise <- mean(in_equip, na.rm = TRUE) * 100

      message("\nEquipoise Diagnostics:")
      message("  Preference score range: [",
              sprintf("%.3f", min(pref_scores, na.rm = TRUE)), ", ",
              sprintf("%.3f", max(pref_scores, na.rm = TRUE)), "]")
      message("  Equipoise bounds:      [",
              equipoise_bounds[1], ", ", equipoise_bounds[2], "]")
      message("  In equipoise:          ",
              sprintf("%.1f%%", pct_equipoise),
              " (", sum(in_equip, na.rm = TRUE), " / ", length(pref_scores), ")")

      equipoise_result <- list(
        preference_scores = pref_scores,
        ps_values = ps_vals,
        prop_treated = prop_treated,
        equipoise_bounds = equipoise_bounds,
        pct_equipoise = pct_equipoise
      )
    }
  }
