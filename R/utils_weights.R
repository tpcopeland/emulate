...
.weight_switch_arm <- function(dt, id_col, treat_col, arm_col, arm_val,
  ...
  # Initialize to NA; set to 1 for first period (no lag) so missing covariates
  # propagate as NA instead of silently defaulting to 1
  dt[in_arm, sw_t := NA_real_]
  dt[in_arm & is.na(lag_treat), sw_t := 1.0]
  ...
  # Default to 1 for extreme PS (near-boundary denominator)
  dt[in_arm & !is.na(lag_treat) & !is.na(denom_pr) &
     (denom_pr <= 0.001 | denom_pr >= 0.999) & is.na(sw_t), sw_t := 1.0]
...
.weight_switch_pooled <- function(dt, id_col, treat_col, arm_col,
  ...
  # Initialize to NA; first period (no lag) gets 1
  dt[, sw_t := NA_real_]
  dt[is.na(lag_treat), sw_t := 1.0]
  ...
  # Default to 1 for extreme PS (near-boundary denominator)
  dt[!is.na(lag_treat) & !is.na(denom_pr) &
     (denom_pr <= 0.001 | denom_pr >= 0.999) & is.na(sw_t), sw_t := 1.0]
